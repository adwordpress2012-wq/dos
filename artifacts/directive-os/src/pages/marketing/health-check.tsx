...
const PACKAGES = [
  { id: "starter",      label: "Starter Rebuild",      price: "$1,800" },
  { id: "business",     label: "Business Rebuild",     price: "$3,500" },
  { id: "professional", label: "Professional Rebuild", price: "$5,200" },
  { id: "enterprise",   label: "Enterprise Rebuild",   price: "$7,500+" },
];
...
function Stars({ n }: { n: number }) {
  return <span>{[1,2,3,4,5].map(i => <span key={i} style={{ color: i <= n ? "#f59e0b" : "#cbd5e1", fontSize: 13 }}>★</span>)}</span>;
}
...
export default function HealthCheck() {
  ...
  const scoreColor = avg >= 7.5 ? "#10b981" : avg >= 5 ? "#f59e0b" : "#ef4444";

  const inp = (edit: boolean): React.CSSProperties => ({
    background: "transparent", border: "none",
    borderBottom: edit ? `1px dashed ${TEAL}` : "none",
    outline: "none", color: "inherit", font: "inherit",
    width: "100%", cursor: edit ? "text" : "default",
  });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "Inter, sans-serif", color: DARK }}>
      ...
      {/* Sticky top bar */}
      <div className="np" style={{ padding: "12px 20px", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", gap: 12, background: "#fff", position: "sticky", top: 0, zIndex: 10, flexWrap: "wrap", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
        <Link href="/marketing" style={{ color: "#64748b", fontSize: 13, textDecoration: "none" }}>← Marketing</Link>
        <span style={{ color: "#e2e8f0" }}>|</span>
        <span style={{ fontWeight: 700, fontSize: 13, color: DARK }}>Website Health Check Builder</span>
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={() => setEditing(!editing)} style={{ padding: "6px 14px", borderRadius: 7, border: `1px solid ${editing ? TEAL : "#e2e8f0"}`, background: editing ? "rgba(0,209,178,0.08)" : "#fff", color: editing ? TEAL : "#64748b", fontWeight: 600, cursor: "pointer", fontSize: 12 }}>
      ...
      <div className="print-outer-grid" style={{ display: "grid", gridTemplateColumns: "290px 1fr", minHeight: "calc(100vh - 49px)" }}>
        ...
        {/* LEFT — sliders + notes */}
        <div className="np" style={{ borderRight: "1px solid #e2e8f0", overflowY: "auto", maxHeight: "calc(100vh - 49px)", position: "sticky", top: 49, padding: "18px 16px", background: "#fff" }}>
          <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 2, color: DARK }}>Rate Each Section</div>
          <div style={{ fontSize: 11, color: "#94a3b8", marginBottom: 16 }}>Drag sliders 1–10 · type notes below each</div>
            ...
            return (
              <div key={s.id} style={{ marginBottom: 18 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: "#475569" }}>{s.icon} {s.label}</span>
                ...
                <textarea value={notes[s.id]} onChange={e => setNotes(p => ({ ...p, [s.id]: e.target.value }))}
                  rows={2} placeholder="Add notes..." style={{ width: "100%", marginTop: 5, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 6, padding: "5px 8px", color: "#64748b", fontSize: 10, lineHeight: 1.5, resize: "vertical", fontFamily: "Inter, sans-serif", boxSizing: "border-box" }} />
              </div>
            );
          })}

          {/* Live score card */}
          <div style={{ background: "#f8fafc", border: `1px solid rgba(0,209,178,0.3)`, borderRadius: 10, padding: "14px", marginTop: 4 }}>
            <div style={{ fontSize: 10, color: "#94a3b8" }}>Overall Score</div>
            <div style={{ fontSize: 30, fontWeight: 900, color: scoreColor }}>{avg}<span style={{ fontSize: 14, fontWeight: 400, color: "#94a3b8" }}>/10</span></div>
        ...
        <div className="print-right-panel" style={{ overflowY: "auto", padding: "24px 20px 60px" }}>
          <div className="page" style={{ maxWidth: 720, margin: "0 auto", background: "#fff", borderRadius: 12, overflow: "hidden", boxShadow: "0 2px 24px rgba(0,0,0,0.08)", border: "1px solid #e2e8f0" }}>

            {/* Report header — dark branded top */}
            <div style={{ background: NAVY, padding: "28px 40px 22px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <img src="/logo.png" alt="Directive OS" style={{ width: 44, height: 44, objectFit: "contain", filter: "drop-shadow(0 0 8px rgba(0,209,178,0.4))" }} />
                  <div>
                    <div style={{ fontWeight: 800, fontSize: 16, color: "#fff" }}>Directive OS</div>
                    <div style={{ color: TEAL, fontSize: 8, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase" }}>Web Solutions & AI Services</div>
                  </div>
                </div>
                <div style={{ textAlign: "right", fontSize: 10, color: "#64748b", lineHeight: 2 }}>
                  <div style={{ color: "#94a3b8" }}>{DATE}</div>
                  <div style={{ color: "#94a3b8" }}>Ref: {REF}</div>
                  <div style={{ color: TEAL }}>directiveos.com.au</div>
                </div>
              </div>

              <div style={{ marginTop: 20, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.08)" }}>
                <div style={{ fontSize: 9, color: TEAL, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.18em", marginBottom: 5 }}>Website Health Check Report — Prepared For</div>
                ...
                <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 3, display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <span>🌐 <input readOnly={!editing} value={clientUrl} onChange={e => setClientUrl(e.target.value)} style={{ ...inp(editing), display: "inline", width: 200, color: "#94a3b8" }} /></span>
                  <span>· Attn: <input readOnly={!editing} value={contactName} onChange={e => setContactName(e.target.value)} style={{ ...inp(editing), display: "inline", width: 160, color: "#94a3b8" }} /></span>
                </div>
              </div>
            </div>

            {/* Teal accent bar */}
            <div style={{ height: 4, background: `linear-gradient(90deg, ${TEAL}, #00b4a0)` }} />

            {/* Executive summary — white */}
            <div style={{ padding: "22px 40px", borderBottom: "1px solid #f1f5f9", display: "flex", alignItems: "center", gap: 24, flexWrap: "wrap", background: "#fff" }}>
              <div style={{ textAlign: "center", minWidth: 80 }}>
                <div style={{ fontSize: 52, fontWeight: 900, lineHeight: 1, color: scoreColor }}>{avg}</div>
                <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 2 }}>out of 10</div>
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 12px", borderRadius: 100, background: rec.color + "15", border: `1px solid ${rec.color}40`, marginBottom: 8 }}>
                  <span style={{ width: 7, height: 7, borderRadius: "50%", background: rec.color, flexShrink: 0, display: "inline-block" }} />
                  <span style={{ color: rec.color, fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase" }}>{rec.verdict}</span>
                </div>
                <div style={{ fontSize: 12, color: "#64748b", lineHeight: 1.7 }}>{rec.summary}</div>
              </div>
            </div>

            {/* Section audit — white with light borders */}
            <div className="print-section" style={{ padding: "24px 40px", borderBottom: "1px solid #f1f5f9", background: "#fff" }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: TEAL, textTransform: "uppercase", letterSpacing: "0.14em", marginBottom: 16, display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ display: "inline-block", width: 24, height: 2, background: TEAL, borderRadius: 2 }} />
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {SECTIONS.map(s => {
                  ...
                  return (
                    <div key={s.id} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderLeft: `3px solid ${c}`, borderRadius: 8, padding: "12px 16px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                        <span style={{ fontSize: 15 }}>{s.icon}</span>
                        <span style={{ fontWeight: 700, fontSize: 12, flex: 1, color: DARK }}>{s.label}</span>
                        <span style={{ fontSize: 9, fontWeight: 700, color: c, background: c + "18", border: `1px solid ${c}30`, borderRadius: 20, padding: "2px 8px" }}>{lbl}</span>
                        <span style={{ fontSize: 16, fontWeight: 900, color: c, minWidth: 38, textAlign: "right" }}>{sc}/10</span>
                      </div>
                      <ScoreBar score={sc} />
                      <div style={{ marginTop: 7, fontSize: 11, color: "#64748b", lineHeight: 1.65 }}>{notes[s.id]}</div>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="print-section" style={{ padding: "24px 40px", borderBottom: "1px solid #f1f5f9", background: "#fff" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {PACKAGES.map(pkg => {
                  ...
                  return (
                    <div key={pkg.id} style={{ background: isR ? "rgba(0,209,178,0.05)" : "#f8fafc", border: `1px solid ${isR ? TEAL : "#e2e8f0"}`, borderRadius: 9, padding: "14px 16px", position: "relative" }}>
                      {isR && <div style={{ position: "absolute", top: -10, right: 10, background: TEAL, color: "#000", fontSize: 8, fontWeight: 800, padding: "2px 10px", borderRadius: 20, textTransform: "uppercase", letterSpacing: "0.1em" }}>★ Recommended</div>}
                      <div style={{ fontWeight: 700, fontSize: 12, color: DARK, marginBottom: 3 }}>{pkg.label}</div>
                      <div style={{ fontSize: 20, fontWeight: 900, color: isR ? TEAL : DARK, marginBottom: 6 }}>{pkg.price}</div>
                      <Stars n={r.stars} />
              ...
              {suggestedAddons.length > 0 && (
                <div style={{ marginTop: 14, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 9, padding: "14px 16px" }}>
                  <div style={{ fontSize: 10, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 10 }}>Also Recommended — Based on Audit Findings</div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {suggestedAddons.map(id => (
                      <span key={id} style={{ fontSize: 10, color: TEAL, background: "rgba(0,209,178,0.08)", border: "1px solid rgba(0,209,178,0.25)", borderRadius: 6, padding: "4px 10px" }}>
                        {ADDON_LABELS[id] || id}
                      </span>
                    ))}
                  </div>
            ...
            {/* CTA — screen only */}
            <div className="np" style={{ padding: "18px 40px", background: "#fffbeb", borderBottom: "1px solid #fde68a", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
              <div>
                <div style={{ fontWeight: 700, fontSize: 13, color: DARK, marginBottom: 2 }}>Ready to see the numbers?</div>
                <div style={{ fontSize: 11, color: "#92400e" }}>This button opens the quote builder pre-filled based on this report.</div>
              </div>
            ...
            {/* Footer */}
            <div style={{ padding: "14px 40px", background: NAVY, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 6, alignItems: "center" }}>
