...
import {
  Users, Building2, Mic, FileText, Flame, MessageSquare,
  Phone, Clock, ChevronRight, Wifi,
} from "lucide-react";
      ...
      <div className="grid lg:grid-cols-3 gap-6 mb-8">
        <div className="bg-card border border-border rounded-xl p-6">
          ...
          <div className="w-full bg-muted rounded-full h-2">
            <div
              className={`h-2 rounded-full transition-all ${aiPct >= 90 ? "bg-red-500" : aiPct >= 70 ? "bg-amber-500" : "bg-primary"}`}
              style={{ width: `${aiPct}%` }}
            />
          </div>
          ...
          {breakdownLoading ? (
            ...
            <div className="flex items-center gap-6">
              <ResponsiveContainer width="55%" height={150}>
                <PieChart>
                  <Pie data={pieData} cx="50%" cy="50%" innerRadius={35} outerRadius={62} paddingAngle={2} dataKey="value">
                    {pieData.map((_, index) => (
              ...
              {recentLeads.slice(0, 6).map(lead => (
                <div key={lead.id} className="flex items-center justify-between py-2.5 border-b border-border/50 last:border-0">
                  <div className="flex items-center gap-3 min-w-0">
                    {lead.channel === "voice" ? <Phone className="w-3.5 h-3.5 text-purple-400 flex-shrink-0" /> : <MessageSquare className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />}
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-foreground truncate flex items-center gap-1.5">
                        {lead.name}
                        {lead.hotLead && <Flame className="w-3 h-3 text-orange-400 inline" />}
                      </div>
