...
import {
  Users, Inbox, Building2, Mic, FileText, Flame, MessageSquare,
  Phone, Clock, ChevronRight, FlaskConical, Database, Brain,
  Wifi, MapPin, ArrowRight,
} from "lucide-react";
...
function SimulationBanner() {
  ...
  if (!status || status.mode !== "SIMULATION") return null;

  const mv = status.mockVaultSummary;
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className="font-semibold text-foreground">SIMULATION MODE</span>
            <span className="text-xs bg-amber-500/20 text-amber-400 border border-amber-500/30 px-2 py-0.5 rounded font-mono font-medium">
              DATABASE_MODE=SIMULATION
            </span>
          </div>
          <p className="text-sm text-muted-foreground mb-4">
            All data is generated by the Directive OS Mock VaultRE Bridge. Demonstrating a live platform snapshot — ready to connect to your production CRM when you are.
          </p>
          {mv && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { icon: <Building2 className="w-3.5 h-3.5" />, label: "Mock Listings", value: String(mv.totalListings) },
                { icon: <MapPin className="w-3.5 h-3.5" />, label: "NSW Suburbs", value: String(mv.suburbs.length) },
                { icon: <Brain className="w-3.5 h-3.5" />, label: "AI Engine", value: status.aiEngine },
                { icon: <Database className="w-3.5 h-3.5" />, label: "VaultRE Bridge", value: "Simulated" },
              ].map(item => (
                <div key={item.label} className="bg-card border border-border rounded-lg px-3 py-2">
                  <div className="flex items-center gap-1.5 text-amber-400 mb-1">{item.icon}<span className="text-xs text-muted-foreground">{item.label}</span></div>
          ...
          {mv && (
            <div className="mt-3 text-xs text-muted-foreground">
              Mock suburbs: {mv.suburbs.slice(0, 6).join(", ")}{mv.suburbs.length > 6 ? ` +${mv.suburbs.length - 6} more` : ""}
            </div>
          )}
        </div>
