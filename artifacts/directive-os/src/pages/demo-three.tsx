...
function LanguageMarquee() {
  ...
  return (
    <div style={{ overflow: "hidden", position: "relative", borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}`, background: "#060810", padding: "18px 0" }}>
      <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 80, background: `linear-gradient(90deg, #060810, transparent)`, zIndex: 2 }} />
      <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 80, background: `linear-gradient(-90deg, #060810, transparent)`, zIndex: 2 }} />
      <div style={{
        ...
        {items.map((l, i) => (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0, padding: "6px 18px", borderRadius: 40, background: "rgba(0,209,178,0.06)", border: `1px solid ${TEAL}22` }}>
            <span style={{ fontSize: 22 }}>{l.flag}</span>
            <div>
              <div style={{ fontSize: 13, fontWeight: 800, color: WHITE, lineHeight: 1 }}>{l.native}</div>
              <div style={{ fontSize: 10, color: TEAL, letterSpacing: 0.5, lineHeight: 1.4 }}>{l.lang}</div>
            </div>
          </div>
        ))}
      </div>
...
function MultilingualSection() {
  return (
    <section style={{ background: DARK_BG, padding: "80px 32px", borderBottom: `1px solid ${BORDER}` }}>
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 52 }}>
          <div style={{
            ...
            color: TEAL, padding: "6px 20px", borderRadius: 100, fontSize: 11, fontWeight: 700,
            marginBottom: 20, letterSpacing: 3, textTransform: "uppercase"
          }}>🌏 9 Languages — Included in Every Plan</div>

          <h2 style={{ fontSize: "clamp(26px, 4vw, 44px)", fontWeight: 900, color: WHITE, margin: "0 0 16px", lineHeight: 1.1 }}>
            Sarah speaks your buyer's language.<br />
            <span style={{ color: TEAL }}>No extra charge. No setup.</span>
          </h2>
          <p style={{ fontSize: 16, color: "#666", lineHeight: 1.8, maxWidth: 600, margin: "0 auto" }}>
            Over 40% of property buyers in Greater Sydney speak a language other than English at home.
            Sarah detects their language instantly and switches — on voice and live chat — so your agency never loses a lead to a language barrier.
          </p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 40 }}>
          {LANGUAGES.map(l => (
            ...
            >
              ...
              <div style={{ fontSize: 13, fontWeight: 800, color: WHITE }}>{l.lang}</div>
              <div style={{ fontSize: 12, color: TEAL, fontWeight: 600 }}>{l.native}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          {[
            { icon: "🎙️", title: "Voice auto-switches", desc: "Sarah detects the caller's language in the first sentence and responds natively — no menu prompts, no hold music, no transfers." },
            { icon: "💬", title: "Chat auto-switches", desc: "The web chat widget detects the typed language and switches immediately. Arabic, Korean, Vietnamese — all in real time." },
            { icon: "🆓", title: "Included at no charge", desc: "All 9 languages are built into every plan. Not an add-on. Not a premium tier. Just part of what Sarah does." },
          ].map(p => (
            <div key={p.title} style={{
              ...
              <div style={{ fontSize: 13, color: "#666", lineHeight: 1.75 }}>{p.desc}</div>
            </div>
          ))}
        </div>
      </div>
    ...
    <>
      {open && (
        <div style={{
          ...
          background: DARK_BG, borderRadius: 14, boxShadow: `0 8px 40px rgba(0,209,178,0.25)`,
          ...
          <div style={{ background: `linear-gradient(90deg, ${TEAL_DARK}, #0891b2)`, padding: "14px 16px", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.7)", marginBottom: 5 }}>Directive OS · Always available</div>
              <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                {LANGUAGES.map(l => (
                  <span key={l.lang} style={{ fontSize: 9, background: "rgba(0,0,0,0.25)", border: "1px solid rgba(255,255,255,0.25)", color: "rgba(255,255,255,0.9)", borderRadius: 4, padding: "1px 5px", fontWeight: 600, letterSpacing: 0.3, whiteSpace: "nowrap" }}>{l.flag} {l.native}</span>
                ))}
              </div>
            </div>
            <button onClick={() => setOpen(false)} style={{ background: "none", border: "none", color: WHITE, fontSize: 22, cursor: "pointer", fontWeight: 700, flexShrink: 0 }}>×</button>
          </div>
          ...
          <div style={{ padding: "12px 14px", borderTop: `1px solid ${BORDER}`, display: "flex", gap: 8 }}>
            <input
              ...
              placeholder="Try any language — Sarah adapts..."
...
export default function DemoThreePage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: DARK_BG, color: WHITE, fontFamily: "'Inter', -apple-system, sans-serif" }}>
      ...
      {/* DEMO BANNER */}
      <div style={{
        background: "#050709", color: "#888", textAlign: "center",
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 840, margin: "0 auto", position: "relative" }}>
          <div style={{
            ...
            AI Receptionist · Available 24/7 · 9 Languages
          </div>
          <h1 style={{ fontSize: "clamp(36px, 5.5vw, 64px)", fontWeight: 900, lineHeight: 1.05, marginBottom: 24, letterSpacing: -1.5, color: WHITE }}>
            Your Office Answers<br />
            <span style={{ color: TEAL }}>Every Call. Every Language.</span>
          </h1>
          <p style={{ fontSize: 19, color: "#888", lineHeight: 1.8, maxWidth: 640, margin: "0 auto 16px" }}>
            Sarah is your AI receptionist — answering every call to your office line, in any language, 24 hours a day. She qualifies the enquiry and sends you the full lead the moment the call ends.
          </p>
          <p style={{ fontSize: 15, color: "#444", lineHeight: 1.7, maxWidth: 560, margin: "0 auto 50px" }}>
            No voicemail. No missed leads. No language barriers.
          </p>
      ...
      {/* HOW IT WORKS */}
      <section id="how-it-works" style={{ background: "#050709", padding: "80px 32px", borderBottom: `1px solid ${BORDER}` }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <p style={{ color: "#555", fontSize: 15, marginTop: 14, maxWidth: 520, margin: "14px auto 0" }}>
              You don't have to do anything differently. The whole thing runs automatically.
            </p>
          </div>
            ...
            {[
              {
                ...
                body: "A buyer, vendor, or tenant rings your office line — midday, at 7pm, on a Saturday — in any language. Instead of ringing out or hitting voicemail, Sarah answers in under 3 seconds.",
              },
              {
                num: "02",
                icon: "🤖",
                title: "Sarah handles the call — in their language",
                body: "Sarah detects the caller's language in the first sentence and switches instantly. Mandarin buyer? She responds in Mandarin. Arabic tenant? Arabic. No menu, no hold, no transfer. Fluent from the first word.",
              },
              {
                num: "03",
                icon: "📧",
                title: "Full transcript lands in Agent's inbox",
                body: "The second the call ends, a lead summary hits your inbox. Caller's name, phone, what they're after — plus the full transcript translated into English, even if the call was in another language.",
                mockup: "email",
              },
              {
                num: "04",
                ...
                body: "Your phone buzzes even if you're at an auction. Tap to open the full lead with context — name, number, what they said, their language, their budget. Call them back knowing exactly who they are.",
                mockup: "app",
              },
            ].map((step, i) => (
                  ...
                  {step.mockup === "email" && (
                    <div style={{ background: "#0d1117", border: `1px solid ${BORDER}`, borderRadius: 12, padding: "20px 24px", maxWidth: 520, fontFamily: "monospace" }}>
                      <div style={{ fontSize: 11, color: "#3a4a5a", marginBottom: 16, borderBottom: `1px solid ${BORDER}`, paddingBottom: 12, fontFamily: "sans-serif" }}>
                        ...
                        <div style={{ color: TEAL, fontWeight: 700, fontSize: 13, fontFamily: "sans-serif" }}>🔴 New Lead — Buyer Enquiry · 🇨🇳 Mandarin Call</div>
                      </div>
                      <div style={{ fontSize: 13, color: "#4a5568", lineHeight: 2, fontFamily: "sans-serif" }}>
                        <div><span style={{ color: "#3a4a5a" }}>Language: </span><span style={{ color: TEAL, fontWeight: 700 }}>Mandarin (普通话) — Translated by Sarah AI</span></div>
                        <div><span style={{ color: "#3a4a5a" }}>Caller: </span><span style={{ color: "#c9d1d9", fontWeight: 600 }}>Wei Zhang</span></div>
                        <div><span style={{ color: "#3a4a5a" }}>Phone: </span><span style={{ color: TEAL, fontWeight: 700 }}>0412 xxx xxx</span></div>
                        <div><span style={{ color: "#3a4a5a" }}>Enquiry: </span><span style={{ color: "#8b949e" }}>3-bed house, Parramatta area, budget $850K</span></div>
                        <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${BORDER}` }}>
                          <div style={{ color: "#3a4a5a", marginBottom: 6, fontSize: 11, letterSpacing: 1 }}>TRANSCRIPT (English Translation)</div>
                          ...
                          <div style={{ fontSize: 12 }}><span style={{ color: "#555" }}>Wei: </span><span style={{ color: "#3a4a5a" }}>Hi, I'm looking for a 3-bedroom property...</span></div>
                    ...
                    <div style={{ maxWidth: 300 }}>
                      <div style={{ background: "#1c1c1e", borderRadius: 16, padding: "14px 16px", marginBottom: 12, border: `1px solid ${BORDER}`, display: "flex", gap: 12, alignItems: "flex-start" }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 12, color: "#6b7280", lineHeight: 1.55 }}>Mandarin call · Pre-approved · 3bd $850K · Tap for English transcript</div>
                      ...
                      <div style={{ background: "#0d1017", border: `1px solid ${TEAL}33`, borderRadius: 12, padding: "16px 18px" }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                          <span style={{ fontWeight: 700, color: WHITE, fontSize: 15 }}>Wei Zhang 🇨🇳</span>
                          <span style={{ background: "#4ade8022", color: "#4ade80", fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 20 }}>Pre-approved</span>
                        </div>
                        <div style={{ fontSize: 12, color: "#4a5568", marginBottom: 12 }}>0412 xxx xxx · 3bd · $850K · Parramatta</div>
      ...
      {/* REASSURANCE */}
      <section style={{ background: DARK_BG, padding: "64px 32px", borderBottom: `1px solid ${BORDER}` }}>
      ...
      {/* CHAT DEMO */}
      <section id="chat-demo" style={{ background: "#050709", padding: "80px 32px", borderBottom: `1px solid ${BORDER}` }}>
        <div style={{ maxWidth: 660, margin: "0 auto", textAlign: "center" }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 3, color: TEAL, marginBottom: 16 }}>TRY IT NOW</div>
          <h2 style={{ fontSize: "clamp(26px, 3.5vw, 40px)", fontWeight: 900, color: WHITE, marginBottom: 16 }}>
            Talk to Sarah in any language
          </h2>
          <p style={{ color: "#555", fontSize: 15, lineHeight: 1.75, marginBottom: 16 }}>
            Hit the chat button — bottom right corner — and try her in Mandarin, Arabic, Vietnamese, or any language. She detects and switches instantly. Or call the number and speak to her live.
          </p>
          <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap", marginBottom: 24, fontSize: 14, color: "#555" }}>
            {LANGUAGES.slice(1).map(l => (
              <span key={l.lang} style={{ padding: "4px 12px", borderRadius: 20, background: DARK_CARD, border: `1px solid ${BORDER}` }}>
                {l.flag} {l.lang}
              </span>
            ))}
          </div>
