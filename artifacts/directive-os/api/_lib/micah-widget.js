/**
 * Mirrors artifacts/api-server/src/lib/micah-widget.ts for Vercel serverless routes.
 */
...
function normaliseProfile(raw) {
  ...
  const str = (k, fb) => (typeof raw[k] === "string" && String(raw[k]).trim() ? String(raw[k]).trim() : fb);
  return {
    business_name: str("business_name", str("name", FALLBACK.business_name)),
    tagline: str("tagline", FALLBACK.tagline),
    opening_hours: str("opening_hours", FALLBACK.opening_hours),
    menu_summary: str("menu_summary", str("menu", FALLBACK.menu_summary)),
    address: str("address", FALLBACK.address),
    phone_display: str("phone_display", str("phone", FALLBACK.phone_display)),
  };
}

export async function fetchSupabaseBusinessProfile(clientId) {
...
function buildMicahDemoSystemPrompt(profile) {
...
- Do not escalate or "hand off" early.
...
function buildDosHubSystemPrompt() {
...
Be concise, friendly, non-technical, and business-focused.

Rules:
- Answer questions about what DOS includes and how it helps (more bookings, fewer missed customers, faster replies).
- If they want pricing, summarise Founding $197/mo, Starter $297/mo, Growth $497/mo, optional website rebuilds from about $1000+, and offer https://calendly.com/adwordpress2012/directive-os-agency-onboarding
- NEVER say "Thanks, I've sent your request to the team" on this chat.
- Do not mention internal voice infrastructure codenames — say "AI voice receptionist" under Micah / Directive OS only.`;
}
...
function hasClosingLine(messages) {
  return transcriptText(messages).toLowerCase().includes("sent your request to the team");
}

function bookingFieldsSatisfied(text) {
...
export async function handleWidgetMessage(body) {
  const clientId = (body?.client_id && String(body.client_id).trim()) || "micah-demo";
  ...
  if (clientId === "dos-hub") {
    ...
    const rawProfile = clientId === "micah-demo" ? await fetchSupabaseBusinessProfile("micah-demo").catch(() => null) : null;
  ...
  try {
    ...
    if (!resp.ok) throw new Error("openai");
    const data = await resp.json();
    reply = data?.choices?.[0]?.message?.content?.trim() || reply;
  } catch {
    reply =
      clientId === "dos-hub"
        ? "No worries — book a free call at https://calendly.com/adwordpress2012/directive-os-agency-onboarding or phone 02 5850 4038."
  ...
  if (clientId !== "dos-hub") {
    ...
    if (mentionsClosing && !satisfied) {
      reply = assistantDraft
        ...
        .trim();
      if (!reply) {
        reply = "Happy to help with a booking — what date works best for you?";
      }
...
export async function handleInstantQuote(body) {
  const payload = {
